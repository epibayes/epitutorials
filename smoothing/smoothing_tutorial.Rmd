...
output$residPlotly <- renderPlotly({
  g <- ggplot(data.frame(x <- data_w_model()$resid)) + geom_histogram(aes(x=x), bins=12)+ xlab("data_w_model()$resid")+ ylab("Frequency")
  p <- ggplotly(g)
  return(p)
})
...
output$diffHistRandomPlotly <- renderPlotly({
  ...
  g <- ggplot(data.frame(x = cs_smooth$z)) + geom_histogram(aes(x=x, y=..density..), bins=100)+ xlab("cs_smooth$z")
  p <- ggplotly(g)
  return(p)

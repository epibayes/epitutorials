...
description: "An Exploration of Smoothing in Modeling"
---
...
require(truncnorm)
require(waiter)
library(reshape2) #for plotly geom_raster?
...
output$residPlotly <- renderPlotly({
  g <- ggplot(data.frame(x <- data_w_model()$resid)) + geom_histogram(aes(x=x, y=..density..), bins=12)+ xlab("data_w_model()$resid") + ylab("Density")
  p <- ggplotly(g)
  return(p)
})
...
output$distPlot <- renderPlot({
  ...
  if (is.na(xpt()) == FALSE) {
    g <- g+ geom_vline(xintercept = xpt(), colour ="darkgray", size = 1) + 
      geom_line(aes(y=subset_pred), colour = "blue", linetype ="dashed", size = 1)
  }
...
fluidRow(
  column(12,
         tabsetPanel(
           ...
           tabPanel("Residuals", plotlyOutput("residPlotly")),
...
fluidRow(
  column(12,
         ...
         selectInput("smoother_degree", "Smoother Type:",
                     c("Moving Average" = 0,
                       "Linear" = 1,
                       "Polynomial" = 2)), style="width: 95%; background-color: #f5f5f5; border: 1px solid #e3d3e3; border-radius: 4px; box-shadow: inset 0 1px 1px rgba(0,0,0,0.05); padding: 19px; min-height: 20px; margin-bottom: 20px; margin-left: 20px;"
  )
)
```
...
output$diffPlotRandom <- renderPlotly({
  ...
  cs_smooth$frequency<- cs_smooth$p - cl_smooth$p
  g <- ggplot(cs_smooth, aes(x=x,y=y)) + geom_raster(aes(fill = frequency)) + coord_equal() +  theme(legend.position = "none") 
  return(ggplotly(g))
})
...
output$diffHistRandomPlotly <- renderPlotly({
  ...
  g <- ggplot(data.frame(x = cs_smooth$z)) + geom_histogram(aes(x=x, y=..density..), bins=100)+ xlab("cs_smooth$z") + ylab("Density")
  p <- ggplotly(g)
  return(p)
...
kde2ddf <- function(df, n = 50, lims = c(range(0:10),range(0:10))){
  ...
  colnames(gr) <- c("x","y","frequency")
  gr$p <- gr$frequency/sum(gr$frequency)
  return(gr)
}
...
output$diffPlot <- renderPlotly({
  df <- data_diff()
  g <- ggplot(df, aes(x=x,y=y)) + geom_raster(aes(fill = frequency)) + coord_equal() + theme(legend.position = "none")
  return(ggplotly(g))
})
...
, style="background-color: #f5f5f5; border: 1px solid #e3d3e3; border-radius: 4px; box-shadow: inset 0 1px 1px rgba(0,0,0,0.05); padding: 19px; min-height: 20px; margin-bottom: 20px;")),
column(9,
       tabsetPanel(
         tabPanel("Max Values", plotlyOutput("maxValsHistPlotly")),
         tabPanel("Hotspots", 
                  # plotOutput("hotspotPlot"),
                  plotlyOutput("hotspotPlotly")
                  ),
...
```{r, context="server"}
  ...
  randomDist <- eventReactive(input$simulate, {
    ...
    g <- ggplot(df, aes(x)) + geom_histogram(aes(y=..density..))+ ylab("Density")
    return(ggplotly(g))
  })
  ...
  output$hotspotPlot <- renderPlot({
    ...
    g <- ggplot() + geom_raster(data=df,aes(x=x, y=y, group=factor(hot), fill=factor(hot))) +  geom_point(data=datadf,aes(x=x,y=y, shape=as.factor(case)), size=2, color="lightgrey")
        waitress$close()
    return(g)

  })
  output$hotspotPlotly <- renderPlotly({
            waitress$start()
    ...
    g <- ggplot(df) + geom_raster(aes(x=x, y=y, group = hot, fill = hot)) + geom_point(data = datadf,aes(x=x,y=y, shape=as.factor(case)), size=1, color="lightgrey")
        waitress$close()
    return(ggplotly(g))
  })

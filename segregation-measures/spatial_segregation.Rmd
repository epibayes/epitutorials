...
grid_plot <- function(df) {
  ...
  g <- ggplot(df, aes(x = x, y = y, fill = p)) + 
    geom_tile() + 
    geom_raster() + 
    scale_fill_continuous_sequential(palette="Blues 3", breaks=c(0,0.25,0.50,0.75,1), labels=c(0,0.25,0.50,0.75,1), limits=c(0,1)) +  ##palette comes from colorspace. breaks try to break the colors into more distinguishable stops but this doesnt work. labels try to add a legend but this also isnt working
    theme_bw() + 
    coord_equal() +

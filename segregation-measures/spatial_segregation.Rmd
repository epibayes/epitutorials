...
# Binned Sequential: https://colorspace.r-forge.r-project.org/reference/scale_colour_binned_sequential.html

# blockbreaks <- seq(0, 1, by = 0.05)
...
grid_plot <- function(df) {
  ...
  g <- ggplot(df, aes(x = x, y = y, fill = p)) +
    geom_tile() +
    scale_fill_binned_sequential(palette = "Blues 3", name=stringr::str_wrap("Proportion Unvaccinated", 20), limits=c(0,1), show.limits = TRUE, oob = scales::squish, aesthetics="fill") + ##NOTE: SHOW.LIMITS IS NOT WORKING
    # scale_fill_gradient2(name=stringr::str_wrap("Proportion Unvaccinated", 20), limits=c(0,1), oob=scales::squish) +
    theme_bw() +
    coord_equal() +

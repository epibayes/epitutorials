...
```{r, context="server"}
# x1 for sample mean and standard dev, x to pass into ggplot

x1 <- reactive({rnorm(input$nsamp, input$mean, input$sd)})

x <- reactive({data.frame(x=rnorm(input$nsamp, input$mean, input$sd))})


output$distPlot2 <- renderPlotly({
  g <- ggplot(x(), aes(x)) + geom_histogram(aes(y = ..density..), binwidth= 1)
  return(ggplotly(g))
 })
...
#})
output$samplemean <- renderText({paste0("Sample Mean = ", round(mean(x1()),2))})

output$se <- renderText({paste0("Sample Standard Deviation = ", round(sd(x1()),2))})
...
ssePlot <-
  eventReactive(list(input$nsamp2, input$mean2, input$sd2), {
    ...
    g <-
      ggplot(df)  + geom_errorbar(aes(x = 1, ymin = lowCI, ymax = highCI), size = 2) + geom_point(aes(x = 1, y = mean), size = 2)
    return(ggplotly(g))
  })
...
ci_contains <- eventReactive(list(input$nsamp4, input$mean4, input$sd4),{
  ...
  for (i in 1:1000) {
    sampled_vals <-rnorm(input$nsamp4, mean = input$mean4, sd = input$sd4)
    tvals <- sigr::TInterval(sampled_vals)
    
    if ((input$mean >= tvals$interval_low) && (input$mean4 <= tvals$interval_high)) {
      num_containing <- num_containing + 1
    }
...
bayes_fit <- eventReactive(list(input$mean5, input$sd5), {
  fit <- stan_glm(y ~ 1, data = data.frame(y = x1()))

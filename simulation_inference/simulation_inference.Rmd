...
ssePlot <-
  eventReactive(list(input$mean2, input$sd2), {
    ...
    g <- ggplot(df, aes(x=samples)) + ylim(round(mean(x2()), 2) - 10, round(mean(x2()), 2) + 10) + geom_pointrange(aes(x = samples, ymin = lowCI, ymax = highCI, y = mean), size = 1)

...
```{r, context="server"}
# sse is a function that listens to nsamp and calculates sse.

sse <- eventReactive(input$nsamp, {qt(0.975, df = input$nsamp-1)*(sd(x())/sqrt(input$nsamp))})
  
# ssePlot is a function that listens to the nsamp, mean, and sd and when any of those change, updates a dataframe that holds the mean, lower CI, and upper CI. These will also update if sse updates (basically if nsamp updates).

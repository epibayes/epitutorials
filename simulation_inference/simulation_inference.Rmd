...
sse <- eventReactive(input$nsamp2, {qt(0.975, df = input$nsamp2-1)*(sd(x())/sqrt(input$nsamp2))})
...
# then, it will generate a ggplot-errorbar that uses x as a random base (currently set at 1 for the plot to generate), ymin/maxes based on the low and high CIs, and a geom_point placed at the x (set at 1, the same value passed to the geom_errorbar).

ssePlot <- eventReactive(list(input$nsamp2, input$mean2, input$sd2),{
...
sseText <- eventReactive(list(input$nsamp2, input$mean2, input$sd2), {

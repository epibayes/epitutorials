...
require(ggplot2)
require(plotly)
require(flexdashboard) #for gauge
...
```{r, context="server"}
# x for sample mean and standard dev, x1 to pass into ggplot

x <- reactive({rnorm(input$nsamp, input$mean, input$sd)})

x1 <- reactive({data.frame(x=rnorm(input$nsamp, input$mean, input$sd))})


output$distPlot2 <- renderPlotly({
  g <- ggplot(x1(), aes(x)) + geom_histogram(aes(y = ..density..), binwidth= 1)
  return(ggplotly(g))
 })


#   plot_ly(x=x(), histfunc= 'avg', type="histogram", histnorm = "density") %>% layout(yaxis = list(title="Frequency"),bargap = 0.01)
...
Updating the mean, sample size, and standard deviation will update the confidence intervals below:
...
# )
fluidRow(
  column(12, verbatimTextOutput("sse", placeholder=TRUE))
)
...
# x2 is a function that listens to and rnorms nsamp2, mean2, and sd2
x2 <- reactive({rnorm(input$nsamp2, input$mean2, input$sd2)})


# sse is a function that listens to nsamp2 and calculates sse.
sse <-
  eventReactive(input$nsamp2, {
    qt(0.975, df = input$nsamp2 - 1) * (sd(x2()) / sqrt(input$nsamp2))
  })

# ssePlot is a function that listens to the nsamp2, mean2, and sd2 and when any of those change, updates a dataframe that holds the mean, lower CI, and upper CI. These will also update if sse updates (basically if nsamp2 updates).
# then, it will generate a ggplot-errorbar that uses x as a random base (currently set at 1 for the plot to generate), ymin/maxes based on the low and high CIs, and a geom_point placed at the x (set at 1, the same value passed to the geom_errorbar) with the y coord being the mean.

ssePlot <-
  eventReactive(list(input$nsamp2, input$mean2, input$sd2), {
    df <-
      data.frame(
        mean = round(mean(x2()), 2),
        lowCI = round(mean(x2()) - sse(), 2),
        highCI = round(mean(x2()) + sse(), 2),
        x=1
      )
    
    g <- ggplot(df, aes(x=x)) + ylim(round(mean(x2()), 2) - 70, round(mean(x2()), 2) + 70) + geom_errorbar(aes(ymin = lowCI, ymax = highCI), size = 2) + geom_point(aes(x = x, y= mean, ymin = lowCI, ymax = highCI), size = 1)
    return(ggplotly(g))
  })
...
mainPanel(
  ...
  plotlyOutput("samplemeansPlotly")
)
...
sm_data <- eventReactive(list(input$nsamp3, input$mean3, input$sd3), {
  mean_output <- rep(0, 1000)
  for (i in 1:1000) {
    mean_output[i] <- mean(rnorm(input$nsamp3, mean = input$mean3, sd = input$sd3))
  }
...
# sm_data2 <- reactive({data.frame(x=sm_data())})

# output$samplemeans <- renderPlot({hist(sm_data(), xlab = paste0("Standard error of the mean = ", round(1.67*sd(sm_data()),2)))})

output$samplemeansPlotly <- renderPlotly({
  plot_ly(x=sm_data(), histfunc= 'avg', type="histogram") %>% layout(xaxis = list(title = paste0("<b>Standard Error of the Mean = </b>", round(1.67*sd(sm_data()),2))),bargap = 0.01)
})
# 

# output$samplemeansPlotly <- renderPlotly({
#   g <- ggplot(sm_data2(), aes(x=x)) + geom_histogram(aes(y= ..density..), binwidth=1)
#   
#   return(ggplotly(g))
# })
...
mainPanel(
  h5("Percent of the Time CI Contains True Mean", style="font-weight:bold;"),
  gaugeOutput("cipct"),
  verbatimTextOutput("cipctText", placeholder=TRUE)
))
)
```
...
ci_contains <- eventReactive(list(input$nsamp4, input$mean4, input$sd4),{
  
  num_containing <- 0
  
  mean_val <- 0
  
  # checks if mean4 is N. if NA, mean_val is 0. else, mean_val is input$mean4
  if (is.na(input$mean4)) {
    mean_val = 0
  }
  else {
    mean_val <- input$mean4    
  }
  
  for (i in 1:1000) {
    sampled_vals <-rnorm(input$nsamp4, mean = mean_val, sd = input$sd4)
    tvals <- sigr::TInterval(sampled_vals)
    
    if ((mean_val>= tvals$interval_low) && (mean_val <= tvals$interval_high)) {
      num_containing <- num_containing + 1
    }
    
  }
  
  return(round(100*num_containing/1000,2))
  
})

#observeEvent to re-add 0 if NA
observeEvent(input$mean4,{
  if (is.na(input$mean4)) {
      updateNumericInput(session, "mean4", value=0)
  }

})

output$cipctText <- renderText(paste0("Contains the true mean ", ci_contains(), "% of the time!"))

output$cipct <- renderGauge({
  gauge(ci_contains(),
        min = 0,
        max = 100,
        symbol= "%",
...
mainPanel(
  tabsetPanel(
    tabPanel("Posterior Mean",  plotOutput("posterior_mean")),
    tabPanel("Posterior Standard Deviation", plotOutput("posterior_sigma"))
  )
...
```{r, context = "server"}
bayes_fit <- eventReactive(list(input$mean5, input$sd5), {
...
output$posterior_mean <- renderPlot({
  ...
  plot_title <- ggtitle("Posterior Mean",
                      subtitle="With Medians and 95% Interval")
  mcmc_areas(posterior, 
           pars = c("(Intercept)"), 
           prob = 0.95) + plot_title + geom_vline(xintercept = input$mean5)
})
...
output$posterior_sigma <- renderPlot({
  ...
  mcmc_areas(posterior, 
           pars = c("sigma"), 
           prob = 0.95) + plot_title + geom_vline(xintercept = input$sd5)
})
```
